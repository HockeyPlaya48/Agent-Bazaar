    ...
    <Link href={`/agents/${capability.slug}`} className="block">
      ...
      {capability.mode && (
        <div className="mt-2 flex gap-1">
          {(Array.isArray(capability.mode) ? capability.mode : [capability.mode]).map((m) => (
            <span key={m} className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[10px] font-medium ${
              m === "natural" 
